...
#  12 times 2 is 24
#  --------------------
with open("sample.txt", 'a') as tables: # changing `a`, to a `w` would CREATE a new file each run.exe
    for i in range(2, 13):
        for j in range(1, 13):
            print("{1:>2} times {0} is {2}".format(i, j, i * j), file=tables)
        print("-" * 20, file=tables)

# OP will rerun each time file is run & append to the end of the file as such
